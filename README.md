...
**CoChem-BASE** is the foundational routing engine, environment orchestrator, and Stage 0 bootstrap layer for the **CoChem** computational ecosystem (`CoChem-BASE`, `CoChem-TOPOS`, `CoChem-TORQ`, `CoChem-SpycFit`, `CoChem-SCRIBE`). It provisions isolated computational silos, schedules hardware-aware quantum chemical workloads, enforces strict architectural air-gaps, and manages state synchronization across heterogeneous computing environments (local workstations, WSL2, and High-Performance Computing clusters).

Built to rigorous *ab initio* theoretical standards, CoChem-BASE integrates directly with the Valeev stack (MPQC, TiledArray, Libint) and external electronic structure engines (ORCA 6.1.0, CFOUR 2.1, xTB 6.7.1, PySCF 2.7+). Workflows execute dynamic JAX-compiled tensor computation graphs guided by the **Method Matrix v4 10-Tier Wall-Clock Heuristic (T0–T9)**.
...
All intensive *ab initio* electronic structure computations (ORCA, CFOUR, MRCC), PES active-learning scans, and Sinc-DVR grid calculations must be dispatched via the `HPCDispatcher` to dedicated High-Performance Computing clusters (managed by **Slurm**, **PBS**, or **Torque**) or configured on-premise multi-GPU workstations.

---

## 5. Getting Started & Stage 0 Bootstrap

The primary entry point for configuring and interacting with CoChem-BASE is the interactive Jupyter notebook **`Start_Here.ipynb`**.

### Prerequisites
- **Operating System**: Linux, macOS, or Windows (native or WSL2).
- **Python**: Version 3.11 or higher.
- **Environment Manager**: `conda`, `mamba`, or `micromamba` accessible in system path.

### Step 1: Clone Repository & Open Notebook
```bash
...
### Step 2: Execute Stage 0 Silo Setup (Cell 1 & 2)
1. In `Start_Here.ipynb`, locate **Cell 2 (Silo Setup & Artifact Registry Configuration)**.
2. Click the cell and press `Shift + Enter`.
3. Choose **New Install** to provision a fresh environment, or **Keep previous setup** to validate an existing installation.
4. Specify your persistent artifact directory (defaults to `~/CoChem_Artifacts`).
5. Click **Create & Provision**. The Stage 0 bootstrapper will build the isolated `cochem_base_silo` environment and register the Jupyter kernel.

### Step 3: Switch Active Jupyter Kernel
- In the top-right corner of your Jupyter interface (or VS Code notebook toolbar), click the kernel selection dropdown.
- Select the newly provisioned kernel: **`cochem_base_silo`**.

### Step 4: Launch the Interactive Matrix Dashboard (Cell 3 & 4)
- Execute **Cell 4 (CoChem-BASE Interactive Matrix Dashboard)**.
- The UI dynamically detects host CPU topology, RAM, AVX-512 flags, and NVIDIA CUDA profiles.
- Configure paths to external quantum chemistry binaries (ORCA, xTB, MPI) directly through the graphical dashboard.

### Step 5: Optional AI Agent Collaboration (Cell 5)
- If enabled, initialize the **Antigravity 2.0 Assistant** daemon widget to access AI-driven workflow assistance and calculation troubleshooting.
...
artifact_dir: Path = get_artifact_dir()
scratch_dir: Path = get_scratch_dir()
...
# 2. Dispatch quantum chemical task via HPCDispatcher
with HPCDispatcher(max_workers=4) as dispatcher:
    future = dispatcher.submit(
        command=["orca", "water_opt.inp"],
        work_dir=str(scratch_dir),
        task_id="orca_opt_001",
        timeout_seconds=3600.0,
    )
    # Tasks run asynchronously; inspect task state
    status: TaskStatus = dispatcher.get_status("orca_opt_001")
