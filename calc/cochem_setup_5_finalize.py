...
and auto-heals configuration files. (Suggestion 8)
"""
...
if __name__ == "__main__":
    import sys
    cfg_file = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("cochem_system_config.json")
    if cfg_file.exists():
        cfg = finalize_system_config(cfg_file)
        print(f"Successfully finalized config: {cfg_file}")
