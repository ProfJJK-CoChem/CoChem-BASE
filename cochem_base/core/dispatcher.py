...
class TaskDispatcher:
    ...
    def __init__(self, max_workers: int = None, timeout_seconds: float = 300.0):
        self.task_queue = queue.Queue()
        self.results: Dict[str, Dict[str, Any]] = {}
        self.results_lock = threading.Lock()
        self.max_workers = max_workers or (os.cpu_count() or 4)
        self.timeout_seconds = timeout_seconds
        self.executor: Union[concurrent.futures.ThreadPoolExecutor, None] = None
        self._stop_event = threading.Event()
        self._worker_thread = None

    def submit_task(self, task_id: str, command: Union[str, List[str]]):
    ...
    def _execute_task(self, task_id: str, command: Union[str, List[str]]):
        try:
...
class SubprocessBroker:
    ...
    def run_command(self, command: Union[str, List[str]], timeout: float = None) -> tuple[int, str, str]:
        if isinstance(command, str):
            cmd_args = shlex.split(command)
        else:
            cmd_args = command
