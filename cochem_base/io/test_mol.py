...
class Atom(BaseModel):
    ...
    def validate_atomic_number(cls, v: int) -> int:
        if v <= 0:
            raise ValueError(f"Atomic number must be strictly positive, got {v}")
        return v
...
class Molecule(BaseModel):
    ...
    def from_xyz(cls, xyz_string: str) -> 'Molecule':
        ...
        """
        lines: list[str] = [line.strip() for line in xyz_string.strip().splitlines() if line.strip()]
        if not lines:
            raise ValueError("Empty XYZ string")

        try:
            num_atoms: int = int(lines[0])
        ...
        if len(lines) < 2 + num_atoms:
            raise ValueError(f"XYZ string truncated: expected {num_atoms} atoms but found {max(0, len(lines) - 2)}")

        name: str = lines[1] if len(lines) > 1 else "Unknown"
        ...
        for i in range(2, 2 + num_atoms):
            ...
            if len(parts) >= 4:
                ...
                atomic_number: int = _SYMBOL_TO_Z.get(symbol.capitalize(), 0)

                atom = Atom(symbol=symbol, atomic_number=atomic_number, x=x, y=y, z=z)
                molecule.add_atom(atom)
            else:
                raise ValueError(f"Invalid atom definition at line {i+1}: {lines[i]}")
