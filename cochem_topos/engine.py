...
class ToposEngine:
    ...
    def generate_mmff94_conformer(self) -> str:
        """
        ...
        """
        logger.info(f"[TOPOS] Generating MMFF94 conformer for {self.smiles}")
    ...
    def perform_torsional_scan(self, dihedral_indices: List[int], steps: int = 36) -> List[float]:
        """
        Performs a true MMFF94 forcefield torsional scan over specified dihedral angle indices.
        """
        logger.info(f"[TOPOS] Performing {steps}-step torsional scan on dihedral {dihedral_indices}")
        energies: List[float] = []
        if self.mol is None or self.mol.GetNumConformers() == 0 or len(dihedral_indices) < 4:
            raise ValueError("Molecule lacks conformers or insufficient dihedral indices provided.")
