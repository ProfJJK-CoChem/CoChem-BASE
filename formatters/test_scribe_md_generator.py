...
def test_yaml_frontmatter_and_system_matrix_generation(
    ...
    frontmatter = builder.generate_yaml_frontmatter(metadata=metadata)
    ...
    sys_section = builder.generate_system_matrix_section(system_matrix=system_matrix)
...
def test_dynamic_mermaid_flowchart_synthesis() -> None:
    ...
    flowchart = builder.generate_mermaid_flowchart(active_stages=active_stages)
    ...
    single_chart = builder.generate_mermaid_flowchart(active_stages=["Stage 0.0"])
    assert 'S0["Stage 0.0: Configuration & Resource Guards"]' in single_chart
    assert "-->" not in single_chart

    # 3. Default flowchart when None passed renders all canonical stages
...
def test_gfm_table_pipe_formatting(
    ...
    alignment rows, numeric precision preservation, keyword argument compatibility,
    and empty table fallbacks.
    """
    builder = MarkdownBuilder()

    # 1. Format sample conformer DataFrame with title keyword argument (SRS Contract)
    table_output = builder.format_gfm_table(
        df=sample_conformer_dataframe, title="Conformer Energetic Ranking"
    )
    ...
    # 2. Backward compatible table_title keyword argument
    table_output_legacy = builder.format_gfm_table(
        df=sample_conformer_dataframe, table_title="Legacy Conformer Ranking"
    )
    assert "### Legacy Conformer Ranking" in table_output_legacy

    # 3. Empty DataFrame returns graceful fallback indicator
    empty_df = pd.DataFrame()
    empty_output = builder.format_gfm_table(df=empty_df, title="Empty Table")
    assert "### Empty Table" in empty_output
    assert "*No tabular data available.*" in empty_output

    # 4. None input returns graceful fallback without crashing
...
def test_thermodynamic_insights_injection_and_token_scrubbing() -> None:
    ...
    rendered = builder.inject_thermodynamic_insights(insights_text=raw_insights)
...
def test_audit_warnings_callout_blockquotes_and_hardware_telemetry(
    ...
    multiline warning blockquote handling, empty warnings fallback, and formatting
    of hardware telemetry section with GPU, CPU, and wall-clock execution metrics.
    """
    builder = MarkdownBuilder(output_dir=tmp_path)
    telemetry_data = sample_metadata_and_telemetry["raw_telemetry"]

    # 1. Non-empty warnings formatting with multiline warning string
    warnings = telemetry_data["warnings"] + ["SCF oscillation detected.\nApplied Broyden mixing on step 5."]
    warning_block = builder.format_warning_blockquotes(warnings=warnings)
    assert (
        "> **WARNING**: SCF convergence required dampening on step 4."
        in warning_block
    )
    assert "> **WARNING**: GPU VRAM spike near 85%." in warning_block
    assert "> **WARNING**: SCF oscillation detected." in warning_block
    assert "> Applied Broyden mixing on step 5." in warning_block

    # 2. Empty warnings fallback
    empty_block = builder.format_warning_blockquotes(warnings=[])
    assert (
    ...
    # 3. Hardware telemetry section formatting using telemetry_data keyword argument (SRS Contract)
    telem_section = builder.format_telemetry_section(telemetry_data=telemetry_data)
    assert (
...
def test_non_destructive_timestamped_overwrite_protection(
    ...
    creates timestamped copies with pattern CoChem_User_Guide_*.md, accepts both
    directory and file destination paths, and ensures UTF-8 encoding.
    """
    ...
    # 1. Write initial document using destination_path pointing directly to a file
    initial_content = "# CoChem User Guide - Run 1\n\nInitial computational run."
    file_1 = builder.write_user_guide(
        content=initial_content, destination_path=output_dir / "CoChem_User_Guide.md"
    )
    ...
    file_2 = builder.write_user_guide(
        content=second_content, destination_path=output_dir / "CoChem_User_Guide.md"
    )
...
def test_end_to_end_user_guide_generation(
    ...
    # Execute build_user_guide using data_payload keyword argument (SRS Contract)
    generated_md = builder.build_user_guide(data_payload=payload)
    ...
    # Write document to disk and verify integrity using destination_path keyword argument (SRS Contract)
    written_file = builder.write_user_guide(
        content=generated_md, destination_path=tmp_path / "final_guide" / "CoChem_User_Guide.md"
    )
