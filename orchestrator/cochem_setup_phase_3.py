...
def audit_single_binary(
    ...
    """
    search_dirs = [custom_path] if custom_path else None
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("=" * 75)
            print("COCHEM SETUP PHASE 3: MULTI-TRACK QUANTUM ENGINE DISCOVERY")
            print("=" * 75)
            print(f"Phase ID:        {report.phase_id}")
            print(f"Status:          {report.status.value}")
            print(f"Timestamp UTC:   {report.timestamp_utc}")
            print(f"Fingerprint:     {report.fingerprint.composite_hash}")
            print(f"Components:      {report.fingerprint.component_count} active hashed elements")
            print(f"Artifact Path:   {report.artifact_path}")
            print("-" * 75)
            print("Engine Discovery Matrix:")
            for name, item in report.engines.items():
                ...
                print(f"  [{avail_tag:<9}] [{item.track.value:<13}] {name:<16}{ver}{hash_tag}")
            print("-" * 75)
            print("Container Subsystem:")
            c_cli = f"{report.container.runtime_name} (v{report.container.runtime_version})" if report.container.runtime_name else "None"
            print(f"  Runtime:       {c_cli}")
            print(f"  Air-Gap Flags: {'VALID' if report.container.airgap_flags_valid else 'INVALID'}")
            print(f"  SIF Images:    {len(report.container.discovered_sifs)} discovered")
            print("-" * 75)
