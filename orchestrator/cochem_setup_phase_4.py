...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("=" * 75)
            print("COCHEM SETUP PHASE 4: DYNAMIC SILO GENERATION & DEPENDENCY ISOLATION")
            print("=" * 75)
            ...
            print(f"Python Version:  {report.version_walking.resolved_version}")
            m = report.mendeleev_authority
            print(f"Mendeleev Hook:  C-12={m.monoisotopic_mass:.5f}, C-13={m.c13_mass:.5f} ({m.authority})")
            print(f"IPC Security:    {report.ipc_security.details}")
            print(f"Disk Saved:      ~{report.manifest_filter.disk_space_saved_estimated_mb:.0f} MB")
            print("-" * 75)
            print("Micro-Silo Matrix:")
            for name, item in report.silos.items():
                ...
                print(f"  [{avail_tag:<14}] {heavy_tag} {name:<20}{path_str}")
            print("-" * 75)
