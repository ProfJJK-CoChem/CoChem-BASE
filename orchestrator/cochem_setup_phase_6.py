...
def run_phase_6_audit(
    ...
    if not dry_run:
        ...
        swmr_supported = True
        lock_passed = True
        probe_detail = "Dry-run: Simulated SWMR capabilities."
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("=" * 75)
            print("COCHEM SETUP PHASE 6: DATABASE & BIFURCATED STORAGE PROVISIONING")
            print("=" * 75)
            ...
            print(f"Databases Dir:   {report.storage_paths.databases_directory}")
            print(f"Scratch Dir:     {report.storage_paths.scratch_directory}")
            print(f"Free Disk Space: {report.storage_paths.free_disk_space_gb:.1f} GB")
            print("-" * 75)
            print("Bifurcated Storage Architecture:")
            print(f"  Runtime Active:  {report.storage_paths.runtime_active_db_path}")
            print(f"  SWMR Supported:  {report.swmr_audit.swmr_supported}")
            print(f"  Active Backend:  {report.swmr_audit.backend_selected.value}")
            print(f"  SQLite WAL Path: {report.storage_paths.sqlite_wal_db_path}")
            print(f"  Archive PES Store:{report.storage_paths.archive_pes_db_path}")
            print(f"  QCSchema Ver:    {report.archive_audit.qcschema_version}")
            print(f"  Groups Scaffold: {', '.join(report.archive_audit.groups_created)}")
            print(f"  Lossless Filter: {report.archive_audit.filters_applied.compression} (opts={report.archive_audit.filters_applied.compression_opts}, shuffle={report.archive_audit.filters_applied.shuffle}, fletcher32={report.archive_audit.filters_applied.fletcher32})")
            print(f"  Scaleoffset:     {report.archive_audit.filters_applied.scaleoffset} (Banned: {report.archive_audit.scaleoffset_banned})")
            print("-" * 75)
