...
class ZeroMQDaemon:
    ...
    async def start(self) -> None:
        ...
        self._running = True
        logger.info(f"ZeroMQDaemon started: PUB bound to {self.host}:{self.pub_port}, SUB connected to {self.host}:{self.sub_port}")
    ...
    async def start_sub_listener(self, port: int = 5556) -> None:
        ...
        if not self.sub_socket:
            ...
            self.sub_socket.setsockopt_string(zmq.SUBSCRIBE, "")
        self._running = True
        logger.info(f"SUB listener connected to {self.host}:{self.sub_port}")
