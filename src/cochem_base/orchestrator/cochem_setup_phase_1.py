...
logger = logging.getLogger("cochem_setup_phase_1")
...
def audit_toolchain_binary(name: str, timeout_seconds: float = 5.0) -> ToolchainItem:
    ...
    try:
        ...
        return ToolchainItem(
            ...
            version="Available (output suppressed)",
            is_available=True,
            error_detail=None,
        )
        ...
        return ToolchainItem(
            ...
            version=None,
            is_available=False,
            error_detail=f"Binary '{name}' execution failed (exit code {exc.returncode})",
        )
...
def run_phase_1_audit(
    ...
    if os_profile.is_windows:
        warnings.append("Host is Windows PE: dynamic stack expansion via ulimit unavailable. Using compiler stack flags.")
    elif kernel_limits.degraded_mode:
        warnings.append("Host Linux stack limit could not be raised to >= 64MB. Large wave-functions may segfault.")

    if kernel_limits.vm_max_map_count is not None and kernel_limits.vm_max_map_count < 262144:
        warnings.append(f"vm.max_map_count is {kernel_limits.vm_max_map_count} (recommended >= 262144).")

    toolchains = audit_toolchains(["gcc", "make", "git"])
    for name, item in toolchains.items():
        if not item.is_available:
            warnings.append(f"Toolchain binary '{name}' not found or inaccessible: {item.error_detail}")
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="CoChem Setup Phase 1: Environment Gatekeeper CLI")
    parser.add_argument("--output-dir", "-o", type=str, default=None, help="Custom output directory")
    parser.add_argument("--target-path", "-t", type=str, default=None, help="Target workspace directory")
    ...
    try:
        ...
        if args.json:
            ...
            logger.info("=" * 70)
            logger.info("COCHEM SETUP PHASE 1: ENVIRONMENT GATEKEEPER AUDIT")
            logger.info("=" * 70)
            ...
            logger.info(f"OS Target:       {report.os_profile.system} ({report.os_profile.machine})")
            logger.info(f"WSL Detected:    {report.os_profile.is_wsl}")
            logger.info(f"Filesystem:      {report.filesystem.fs_type} (POSIX Compliant: {report.filesystem.is_posix_compliant})")
            ...
            for name, item in report.toolchains.items():
                ...
                logger.info(f"  [{avail}] {name}: {item.path or item.error_detail}{ver}")
            logger.info("-" * 70)
