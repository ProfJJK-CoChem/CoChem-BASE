        ...
        for node_dir in node_entries:
            ...
            if meminfo_file.exists():
                try:
                    ...
                    node_total_mb = round(psutil.virtual_memory().total / (1024.0 * 1024.0), 2)
                    node_free_mb = round(psutil.virtual_memory().free / (1024.0 * 1024.0), 2)
            ...
            if cpulist_file.exists():
                try:
                    ...
                    cpu_ids = list(range(psutil.cpu_count(logical=True) or 1))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            logger.info("=" * 75)
            logger.info("COCHEM SETUP PHASE 11: MEMORY ROUTER & OOM SHIELD GATEKEEPER")
            logger.info("=" * 75)
            ...
            logger.info("-" * 75)
            logger.info("Host & Container Bounded Memory Profile:")
            hm = report.host_memory
            logger.info(f"  Physical RAM:    {hm.total_ram_bytes / (1024**3):.2f} GB ({hm.available_ram_bytes / (1024**3):.2f} GB available)")
            logger.info(f"  Swap Space:      {hm.swap_total_bytes / (1024**3):.2f} GB ({hm.swap_free_bytes / (1024**3):.2f} GB free)")
            cg = report.cgroup_profile
            logger.info(f"  CGroup Version:  {cg.cgroup_version.value} (Constrained: {cg.is_cgroup_constrained})")
            if cg.memory_limit_bytes:
                logger.info(f"  CGroup Limit:    {cg.memory_limit_bytes / (1024**3):.2f} GB")
            logger.info(f"  Bounded RAM:     {hm.bounded_total_ram_gb:.2f} GB ({hm.bounded_total_ram_mb:.1f} MB)")
            logger.info("-" * 75)
            logger.info("NUMA Multi-Tier Memory Topology:")
            numa = report.numa_profile
            logger.info(f"  NUMA Nodes:      {numa.numa_nodes_count} (Aware: {numa.is_numa_aware}, Balance: {numa.numa_balance_status.value})")
            logger.info(f"  Tier 1 Local:    {numa.tier_1_local_ram_mb / 1024:.2f} GB")
            logger.info(f"  Tier 2 Remote:   {numa.tier_2_remote_ram_mb / 1024:.2f} GB")
            logger.info(f"  Tier 3 Swap:     {numa.tier_3_swap_mb / 1024:.2f} GB")
            logger.info("-" * 75)
            logger.info("OOM Shield Memory Partitioning Profile:")
            oom = report.oom_shield
            logger.info(f"  CPU Cores:       {oom.total_physical_cores} Physical Cores / {oom.active_job_cores} Active Job Core(s)")
            logger.info(f"  OS Safety Buffer: {oom.os_jupyter_reserve_mb} MB ({oom.reserve_ratio * 100:.1f}%) [Flat 4-8 GB Reserved]")
            logger.info(f"  Allocatable RAM: {oom.allocatable_ram_mb} MB ({oom.allocatable_ram_gb:.2f} GB)")
            logger.info(f"  Baseline %maxcore (All Cores): {oom.baseline_80pct_maxcore_mb} MB/core")
            logger.info(f"  OOM Shield %maxcore (Active):  {oom.active_core_maxcore_mb} MB/core")
            logger.info(f"  Memory Gain:     +{oom.memory_gain_vs_baseline_pct:.1f}% vs baseline")
            logger.info("-" * 75)
            logger.info("Multi-Engine Target Directives:")
            for eng_name, bud in report.engine_budgets.items():
                logger.info(f"  [{eng_name:10}] {bud.directive_value_formatted:<35} | {bud.env_var_name}={bud.env_var_value}")
            logger.info("-" * 75)
