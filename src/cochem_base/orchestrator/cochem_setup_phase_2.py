...
logger = logging.getLogger("cochem_setup_phase_2")
...
def get_absolute_physical_ram() -> int:
    ...
    sysconf_names = getattr(os, "sysconf_names", None)
    if callable(sysconf) and isinstance(sysconf_names, dict):
        try:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            logger.info("=" * 70)
            logger.info("COCHEM SETUP PHASE 2: HARDWARE & RESOURCE GATEKEEPER AUDIT")
            logger.info("=" * 70)
            ...
            if report.cpu.cgroup_effective_cpus is not None:
                logger.info(f"cgroup Quota:    {report.cpu.cgroup_effective_cpus} effective CPUs")
            logger.info(
            ...
            for dev in report.gpu.devices:
                ...
                logger.info(f"  - [{dev.vendor}] {dev.name} (VRAM: {mem_gb})")
            logger.info(f"Float Precision: {report.precision.precision_message.split(': ')[-1]}")
