...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            logger.info("=" * 75)
            logger.info("COCHEM SETUP PHASE 9: HETEROGENEOUS PARSL CONCURRENCY EXECUTOR MAPPING")
            logger.info("=" * 75)
            ...
            logger.info("-" * 75)
            logger.info("Scout-and-Anchor Heterogeneous Topology:")
            prof = report.scout_anchor_profile
            logger.info(f"  Physical Cores:  {prof.total_physical_cores} (Logical Threads: {prof.total_logical_cores})")
            logger.info(f"  Anchor Pool:     {prof.anchor_cores} P-cores -> Affinity: {prof.anchor_affinity.affinity_string}")
            logger.info(f"                   Provider: {prof.anchor_executor.provider.value}, Max Workers: {prof.anchor_executor.max_workers_per_node}")
            logger.info(f"  Scout Pool:      {prof.scout_cores} P-core -> Affinity: {prof.scout_affinity.affinity_string}")
            logger.info(f"                   GPU Workers: {prof.scout_gpu_workers}, Measured Step Latency: {prof.scout_step_latency_ms} ms [M]")
            logger.info(f"  Parsl Status:    {'Installed (' + str(prof.parsl_version) + ')' if prof.is_parsl_installed else 'Not Installed'}")
            logger.info("-" * 75)
            logger.info("Anti-Spoofing Amnesty Status:")
            logger.info(f"  Amnesty File:    {report.amnesty_profile.amnesty_file_path}")
            logger.info(f"  Entries Count:   {report.amnesty_profile.total_amnestied_entries}")
            logger.info(f"  Amnesty Valid:   {report.amnesty_profile.is_amnesty_present and report.amnesty_profile.is_alphabetically_sorted}")
            logger.info(f"  Parsl Whitelist: {report.amnesty_profile.parsl_whitelisted}")
            logger.info("-" * 75)
