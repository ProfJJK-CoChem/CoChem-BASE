...
Legacy Naming Note:
'placeholder_values' refers to path token replacements (<USER_HOME>, <COCHEM_WORKSPACE>, <GDRIVE_ROOT>)
used to redact sensitive user filesystem paths in published logs and artifacts.
It does NOT represent mock or stub logic.
"""
...
def get_agents_dir(base_root: Path | None = None) -> Path:
    ...
    if configured:
        p = resolve_mapped_path(configured, get_base_root())
    else:
        root = base_root if base_root is not None else get_base_root()
        p = (root / ".agents").resolve()
