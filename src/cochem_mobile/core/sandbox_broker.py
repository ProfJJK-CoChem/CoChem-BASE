...
class SandboxBroker:
    ...
    def __init__(self, preferred_engine: Optional[ContainerEngine] = None) -> None:
        ...
        elif ContainerEngine.SUBPROCESS in self.available_engines:
            self.active_engine = self.available_engines[0]
        else:
            self.active_engine = ContainerEngine.SUBPROCESS

    @staticmethod
    def detect_available_engines() -> List[ContainerEngine]:
        """Detect which container runtimes are physically installed on host."""
    ...
    def execute(
        ...
        engine = force_engine or self.active_engine
