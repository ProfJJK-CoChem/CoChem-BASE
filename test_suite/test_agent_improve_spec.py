...
def improve_path() -> Path:
    ...
    path = agents_dir / "cochem-improve.agent.md"
    assert path.exists(), f"cochem-improve.agent.md does not exist at {path}"
    return path


def test_improve_file_encoding_and_line_endings(improve_path: Path) -> None:
    """Validate that cochem-improve.agent.md has no UTF-8 BOM and strictly uses Unix LF line endings."""
    raw_bytes = improve_path.read_bytes()
    assert not raw_bytes.startswith(b"\xef\xbb\xbf"), "cochem-improve.agent.md contains UTF-8 BOM"
    assert b"\r\n" not in raw_bytes, "cochem-improve.agent.md contains Windows CRLF line endings"


def test_improve_yaml_frontmatter_schema(improve_path: Path) -> None:
...
def test_improve_path_sanitization_and_no_leaks(improve_path: Path) -> None:
    """Validate that cochem-improve.agent.md contains zero personal path leaks and uses standard tokens."""
    content = improve_path.read_text(encoding="utf-8")
...
def test_improve_mandatory_sections_present(improve_path: Path) -> None:
    """Validate that all mandatory canonical architecture sections exist in cochem-improve.agent.md."""
    content = improve_path.read_text(encoding="utf-8")
    required_sections = [
        "# IDENTITY AND ROLE",
    ...
    for section in required_sections:
        assert section in content, f"Missing required section in cochem-improve.agent.md: {section}"


def test_improve_anti_spoofing_invariants(improve_path: Path) -> None:
    """Validate that cochem-improve.agent.md incorporates critical anti-spoofing and zero-mock protocols."""
    content = improve_path.read_text(encoding="utf-8")
