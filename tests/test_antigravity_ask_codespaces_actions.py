...
def setup_env() -> dict[str, str]:
    ...
    # Ensure GEMINI_API_KEY is not interfering with the interactive input test
    env.pop("GEMINI_API_KEY", None)
    return env

def test_antigravity_ask_codespaces_actions(setup_env):
...
def main():
    ...
    assert os.environ.get("COCHEM_CALCULATION_OS") == "github-actions"

    # Test token stripping directly
    stripped = daemon_instance._guardrail_strip('Optimize this: C 0.00000 0.00000 0.00000')
    print(f"STRIPPED: {stripped}")
    sys.stdout.flush()

    try:
        # Inject token interactively
        daemon_instance.google_oauth_flow()
        # Query API natively - should fail because token is generic
        res = daemon_instance.query('Hello from Codespaces + GitHub Actions')
        print("RESULT:", res)
...
if __name__ == "__main__":
    main()
"""
    artifacts_dir = os.environ.get("COCHEM_ARTIFACTS_DIR", str(pathlib.Path.home() / ".cochem" / "artifacts"))
    pathlib.Path(artifacts_dir).mkdir(parents=True, exist_ok=True)
    
    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.py', dir=artifacts_dir) as f:
    ...
    try:
        subprocess.run(
            ...
            env=setup_env,
            input="GENERIC_NEUTRAL_TOKEN_ACTIONS_TEST\\n",
            capture_output=True,
            text=True,
            timeout=15,
            check=True
        ...
        assert "STRIPPED: Optimize this: C <XYZ_STRIPPED>" in stdout
        ...
        try:
            ...
            print(f"Cleanup warning for {script_path}: {e}")
