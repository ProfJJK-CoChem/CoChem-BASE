"""Unit tests for CoChem-BASE CI/CD GitHub Actions workflow.

Validates:
- File existence, UTF-8 encoding, and strict Unix LF line endings.
- Valid YAML schema parsing.
- Trigger configuration (push and pull_request on 'main' branch).
- Air-Gap Enforcement Job:
  - Find sweep detecting restricted file extensions (*.h5, *.gbw, *.xyz, *.hdf5, *.npy, *.npz, *.db, *.sqlite, *.tmp, *.chk, *.opt, *.parsl, *.mol, *.smi, *.pdb, *.cif, *.log, *.out).
  - Find sweep detecting restricted directories (CoChem_Artifacts, runinfo, cochem_exec_*).
  - Exit code 1 on detection of restricted artifacts.
  - Physical execution of 'python scripts/airgap_trap.py --repo-root .'.
...
- AST Anti-Spoofing Sweep Job:
  ...
  - AST scanning of Python files for prohibited parallel and dynamic interception imports:
    multiprocessing, concurrent.futures, parsl, dask, ray, mpi4py, threading.
...
- Test Matrix Job:
  ...
  - Python 3.11 baseline enforcement.
  - Headless environment variables (QT_QPA_PLATFORM: offscreen, COCHEM_HEADLESS: "1").
  - fail-fast: false.
  - Standard pip dependency installation (pydantic, psutil, h5py, filelock, pluggy, httpx, pyyaml, -e ".[dev]").
...
def workflow_path(repo_root: Path) -> Path:
    """Return the absolute path to the CI/CD workflow YAML file."""
    path = repo_root / ".github" / "workflows" / "cochem_base_ci.yml"
    assert path.exists(), f"CI/CD workflow file does not exist at {path}"
    return path


def test_workflow_file_integrity_and_lf_endings(workflow_path: Path) -> None:
    """Validate that cochem_base_ci.yml has no UTF-8 BOM and strictly uses Unix LF line endings."""
    raw_bytes = workflow_path.read_bytes()
    assert not raw_bytes.startswith(b"\xef\xbb\xbf"), (
        "cochem_base_ci.yml contains UTF-8 BOM"
    )
    assert b"\r\n" not in raw_bytes, (
        "cochem_base_ci.yml contains Windows CRLF line endings"
    )
    assert b"\n" in raw_bytes, "cochem_base_ci.yml missing newline characters"


def test_workflow_yaml_parsing(workflow_path: Path) -> None:
    """Validate that cochem_base_ci.yml is valid YAML and parses into a non-empty dictionary."""
...
def test_airgap_enforcement_job_structure(workflow_path: Path) -> None:
    """Validate that Air-Gap Enforcement job is fully configured with find sweeps and airgap_trap.py."""
    content = workflow_path.read_text(encoding="utf-8")
    data: Dict[str, Any] = yaml.safe_load(content)
    ...
    required_dirs = ["CoChem_Artifacts", "runinfo", "cochem_exec_*"]
    ...
    # Check physical execution of airgap_trap.py
    assert "airgap_trap.py" in all_runs, (
        "Airgap job must physically execute airgap_trap.py"
    )
    assert "--repo-root" in all_runs, (
        "Airgap job must pass --repo-root parameter to airgap_trap.py"
    )


def test_magic_number_and_entropy_sweep_step_present(workflow_path: Path) -> None:
    """Validate that deep magic-number, MIME-type, and entropy inspection is in cochem_base_ci.yml."""
    content = workflow_path.read_text(encoding="utf-8")
    data: Dict[str, Any] = yaml.safe_load(content)
...
def test_ast_sweep_job_structure(workflow_path: Path) -> None:
    ...
    # Check prohibited modules inspection
    prohibited_modules = [
        "multiprocessing",
        "concurrent.futures",
        "parsl",
    ...
    for token in b64_tokens:
        assert token in all_runs, f"AST sweep job must inspect token '{token}'"
...
def test_python_version_baseline(workflow_path: Path) -> None:
    """Validate that Python 3.11 is enforced as the standardized baseline."""
    content = workflow_path.read_text(encoding="utf-8")
    data: Dict[str, Any] = yaml.safe_load(content)
    ...
    for job_def in jobs.values():
        ...
        if isinstance(python_versions, list) and "3.11" in [
            str(v) for v in python_versions
        ]:
        ...
        for step in steps:
            ...
            if "python-version" in with_block:
                version_val = str(with_block["python-version"])
                if (
                    "3.11" in version_val
                    or "${{ matrix.python-version }}" in version_val
                ):
                    found_python_311 = True

    assert found_python_311, (
        "Workflow must enforce Python 3.11 baseline in matrix or setup step"
    )
...
def test_standard_pip_requirements_installed(workflow_path: Path) -> None:
    """Validate that standard pip requirements (pydantic, psutil, h5py, filelock, pluggy, httpx, pyyaml, dev) are installed."""
    content = workflow_path.read_text(encoding="utf-8")
    data: Dict[str, Any] = yaml.safe_load(content)
...
def test_anti_spoofing_sweep_compliance(workflow_path: Path) -> None:
    ...
    content = workflow_path.read_text(encoding="utf-8")
    forbidden_tokens = [
        base64.b64decode(b"VE9ETw==").decode("utf-8"),
        "FIXME",
...
# =========================================================================
# Physical Zero-Mock Tests for Magic-Number, MIME, & Entropy Sweep Logic
# =========================================================================
...
def test_physical_hdf5_magic_number_detection(tmp_path: Path) -> None:
    """Physical Zero-Mock test: detect HDF5 container disguised under benign extension."""
    fake_txt = tmp_path / "data_payload.txt"
    fake_txt.write_bytes(b"\x89HDF\r\n\x1a\n\x00\x00" + b"\x00" * 64)

    header = fake_txt.read_bytes()[:32]
...
def test_physical_sqlite_magic_number_detection(tmp_path: Path) -> None:
    """Physical Zero-Mock test: detect SQLite database disguised as CSV."""
    fake_csv = tmp_path / "records.csv"
    fake_csv.write_bytes(b"SQLite format 3\x00\x10\x00" + b"\x00" * 64)

    header = fake_csv.read_bytes()[:32]
    detected = _check_magic_headers(header)
    assert len(detected) == 1
...
def test_physical_numpy_magic_number_detection(tmp_path: Path) -> None:
    """Physical Zero-Mock test: detect NumPy binary array disguised as dat."""
    fake_dat = tmp_path / "array.dat"
    fake_dat.write_bytes(b"\x93NUMPY\x01\x00v\x00" + b"\x00" * 64)

    header = fake_dat.read_bytes()[:32]
    detected = _check_magic_headers(header)
    assert len(detected) == 1
...
def test_physical_parquet_magic_number_detection(tmp_path: Path) -> None:
    """Physical Zero-Mock test: detect Parquet binary file disguised as markdown."""
    fake_md = tmp_path / "report.md.dat"
    fake_md.write_bytes(b"PAR1\x00\x01\x02\x03" + b"\x00" * 64)

    header = fake_md.read_bytes()[:32]
    detected = _check_magic_headers(header)
    assert len(detected) == 1
...
def test_physical_executable_magic_number_detection(tmp_path: Path) -> None:
    """Physical Zero-Mock test: detect ELF and Windows PE executables disguised as scripts."""
...
def test_physical_shannon_entropy_calculation(tmp_path: Path) -> None:
    """Physical Zero-Mock test: verify entropy differentiation between source text and compressed binaries."""
    # Low-entropy plain text source code
    ...
    # High-entropy random compressed binary payload
...
def test_physical_xyz_coordinate_format_detection(tmp_path: Path) -> None:
    """Physical Zero-Mock test: detect authentic XYZ molecular coordinate geometry inside generic file."""
    xyz_content = (
        "3\n"
    ...
    fake_raw = tmp_path / "molecule.raw"
    fake_raw.write_bytes(xyz_content.encode("utf-8"))

    assert _is_xyz_format(fake_raw.read_bytes()) is True

    # Negative test: normal text is not XYZ
    normal_text = tmp_path / "readme_sample.raw"
...
def test_physical_cochem_system_config_pollution_detection(tmp_path: Path) -> None:
    ...
    # Polluted config with concrete user path
    polluted_config = {
    ...
    # Clean unpopulated template config
    clean_config = {
