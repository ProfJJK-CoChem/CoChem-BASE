...
def test_utf8_encoding_no_bom(target_file: Path) -> None:
    ...
    with open(target_file, "rb") as f:
        header = f.read(3)
    assert header != b"\xef\xbb\xbf", "Found UTF-8 BOM marker in cochem-audit.agent.md"
...
def test_yaml_frontmatter_validity(target_file: Path) -> None:
    ...
    assert "adversary" in data["routes_to"]
    assert data.get("enable_write_tools") is True, "enable_write_tools must be true"
    assert data.get("enable_subagent_tools") is True, "enable_subagent_tools must be true"
