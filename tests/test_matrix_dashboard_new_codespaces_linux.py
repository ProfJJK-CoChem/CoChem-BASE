...
def codespaces_linux_ephemeral_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """
    Simulates a Codespaces interaction + Local-Linux calculation environment
    by pointing the artifact directory to a temporary space and setting variables.
    """
    cs_linux_scratch = tmp_path / "scratch" / "codespaces_linux" / "CoChem_Artifacts"
    cs_linux_scratch.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("COCHEM_ARTIFACT_DIR", str(cs_linux_scratch))
    return cs_linux_scratch


# Provide backward-compatibility fixture name
test_env = codespaces_linux_ephemeral_env


@pytest.mark.skipif(
    os.environ.get("CODESPACES") != "true" or os.environ.get("COCHEM_CALCULATION_OS") != "linux",
    reason="Requires CODESPACES=true and COCHEM_CALCULATION_OS=linux",
)
def test_matrix_dashboard_new_codespaces_linux(
    codespaces_linux_ephemeral_env: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """
    ...
    # Configure calculation target for Local-Linux (Deb)
    gui.calc_target.value = "Local-Linux (Deb)"

    # Verify path resolutions respected our injected environment
    assert str(codespaces_linux_ephemeral_env) in str(gui.artifact_dir)
    ...
    manifest_path = codespaces_linux_ephemeral_env / "Registry" / "cochem_deployment_manifest.json"
    ...
    system_config_path = codespaces_linux_ephemeral_env / "Registry" / "cochem_system_config.json"
    ...
    assert config_data["calculation_tier"] == "Local-Linux (Deb)"
...
test_matrix_dashboard_codespaces_linux_deployment = test_matrix_dashboard_new_codespaces_linux
