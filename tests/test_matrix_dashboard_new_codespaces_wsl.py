...
test_env = codespaces_wsl_ephemeral_env


@pytest.mark.skipif(
    os.environ.get("CODESPACES") != "true" or os.environ.get("COCHEM_CALCULATION_OS") != "wsl",
    reason="Requires CODESPACES=true and COCHEM_CALCULATION_OS=wsl",
)
def test_matrix_dashboard_new_codespaces_wsl(
    codespaces_wsl_ephemeral_env: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """
    ...
    # Configure calculation target for Local-Windows (WSL)
    gui.calc_target.value = "Local-Windows (WSL)"

    # Verify path resolutions respected our injected environment
    assert str(codespaces_wsl_ephemeral_env) in str(gui.artifact_dir)
    ...
    manifest_path = codespaces_wsl_ephemeral_env / "Registry" / "cochem_deployment_manifest.json"
    ...
    assert manifest.calculation_environment == "Local-Windows (WSL)"
    ...
    system_config_path = codespaces_wsl_ephemeral_env / "Registry" / "cochem_system_config.json"
    ...
    assert config_data["calculation_tier"] == "Local-Windows (WSL)"
...
test_matrix_dashboard_codespaces_wsl_deployment = test_matrix_dashboard_new_codespaces_wsl
