...
def test_silo_setup_keep_codespaces_hpc(conda_env_simulator, caplog, capsys):
    ...
    re-creation (the 'Keep previous setup' logic) under Codespaces + HPC simulation.
    """
    ...
    try:
        ...
        pytest.fail(f"Conda executable not found: {e}")
        ...
        with FileLock(str(lock_path), timeout=60):
            for attempt in range(max_retries):
                try:
                    ...
                    logger.error(f"Conda executable not found during creation: {e}")
