...
def test_silo_setup_keep_linux_hpc(hpc_artifact_dir):
    """
    Tests the "Keep previous setup" logic of the Silo Setup module
    targeting Local-Linux/HPC. Physically provisions Conda with `zlib`
    to prepare a "kept" environment. Uses FileLock and retry logic to
    avoid hitting conda's 429 RESOURCE_EXHAUSTED rate limits.
    """
        ...
        with FileLock(str(lock_path), timeout=60):
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    logger.info(
                        f"Attempt {attempt}: Provisioning conda env at "
                        f"{silo_dir}"
                    )
                    subprocess.run(
                        cmd,
    ...
    # Verify that conda-meta exists and contains json files
    conda_meta_dir = silo_dir / "conda-meta"
    assert conda_meta_dir.exists(), (
        "conda-meta directory was not created."
    )
    assert list(conda_meta_dir.glob("*.json")), (
        "No .json files found in conda-meta."
    )

    # Run the setup module via subprocess to enforce zero-mock
    # physical isolation
    env = os.environ.copy()
    ...
    assert "Conda environment already exists at:" in output, (
        "Did not detect existing Conda environment."
    )
    assert "Skipping creation process" in output, (
        "Did not skip creation process."
    )
